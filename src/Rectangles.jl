...
"""
function render(r::Rectangle, ::PlainRectangle, name="main";
...
"""
function render(r::Rectangle, s::RoundedRectangle, name="main";
        layer::Real=0, datatype::Real=0)
    c = cell(name)
    rad = s.r
    ...
    straight(p, r.ury-r.lly-2*rad)
    turn(p, π/2, rad/2)
    render(p, Trace(s.r), name, layer=layer, datatype=datatype)
end
